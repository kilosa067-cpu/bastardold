...
import {
  ...
  MessageCircle, Loader2, Crown, Users
} from 'lucide-react';
...
export default function Agenda() {
  ...
  // Auto-assign barber based on selected services
  useEffect(() => {
    if (selectedServices.length > 0) {
      if (hasBastardServices) {
        const jahzeel = barbers.find(b => b.id === 'bastard');
        if (jahzeel) selectBarber(jahzeel);
      } else {
        const turno = barbers.find(b => b.id === 'turno');
        if (turno) selectBarber(turno);
      }
    }
  }, [selectedServices, hasBastardServices, selectBarber]);
  ...
  const getAvailableTimeSlots = () => {
    ...
    const barberId = selectedBarber?.id || 'turno';
    return getAvailableSlots(selectedDate.toISOString().split('T')[0], barberId);
  };

  const openWhatsApp = () => {
    const barberName = selectedBarber?.name || 'Barbero en turno';
  ...
  if (selectedServices.length === 0) {
    return (
      <section id="agenda" ref={sectionRef} className="relative w-full py-24 md:py-32 bg-[#FAFAFA] overflow-hidden">
        <div className="section-padding relative z-10 max-w-2xl mx-auto text-center">
          <div className="agenda-header">
            <h2 className="font-outfit font-bold text-4xl md:text-5xl text-[#2D5A27] mb-4">Agendar Cita</h2>
            <p className="text-[#333]/60 mb-8">
              Primero selecciona tus servicios en la sección de servicios, luego podrás agendar tu cita aquí.
            </p>
            <Button
  ...
  // Show barber assignment info + Date selection
  if (currentStep === 'barber' || currentStep === 'service' || currentStep === 'date') {
    return (
      <section id="agenda" ref={sectionRef} className="relative w-full py-24 md:py-32 bg-[#FAFAFA] overflow-hidden">
        <div className="section-padding relative z-10">
          <div className="agenda-header text-center mb-12 max-w-3xl mx-auto">
            ...
            <p className="text-[#333]/60">
              Revisa tu selección y elige la fecha para tu cita.
            </p>
          </div>

          <div className="max-w-4xl mx-auto">
            {/* Barber Assignment Card */}
            <div className={`mb-8 p-6 rounded-2xl border-2 ${hasBastardServices ? 'border-[#CE1126] bg-[#CE1126]/5' : 'border-[#2D5A27]/10 bg-white'}`}>
              <div className="flex items-center gap-4">
                <div className={`w-14 h-14 rounded-full flex items-center justify-center ${hasBastardServices ? 'bg-[#CE1126]/10' : 'bg-[#2D5A27]/10'}`}>
                  {hasBastardServices ? (
                    <Crown className="w-7 h-7 text-[#CE1126]" />
                  ) : (
                    <Users className="w-7 h-7 text-[#2D5A27]" />
                  )}
                </div>
                <div className="flex-1">
                  <p className="text-sm text-[#333]/50 mb-1">
                    {hasBastardServices ? 'Servicio exclusivo con' : 'Serás atendido por'}
                  </p>
                  <h3 className="font-outfit font-bold text-xl text-[#2D5A27]">
                    {selectedBarber?.name || 'Barbero en turno'}
                  </h3>
                  {hasBastardServices && (
                    <p className="text-[#CE1126] text-sm">Servicios premium de Bastard Old School</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="mb-8 bg-white rounded-2xl p-6 border border-[#2D5A27]/10">
              <h4 className="font-outfit font-semibold text-[#2D5A27] mb-3">Servicios seleccionados</h4>
            ...
            <div className="bg-white rounded-3xl p-6 md:p-10 border border-[#2D5A27]/10">
              <h3 className="font-outfit font-bold text-xl text-[#2D5A27] mb-6 text-center">Selecciona la Fecha</h3>
  ...
  if (bookingComplete) {
    return (
      <section id="agenda" className="relative w-full py-24 md:py-32 bg-[#FAFAFA]">
