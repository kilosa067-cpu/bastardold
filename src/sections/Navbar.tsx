    ...
    <>
      <nav
        className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 transition-all duration-500 ease-out ${
          isScrolled
            ? 'bg-white/90 backdrop-blur-xl border border-[#2D5A27]/10 shadow-lg py-2 px-4'
        ...
        <div className="flex items-center gap-6">
          ...
          >
            <div className={`w-8 h-8 rounded-full overflow-hidden border-2 transition-colors duration-300 ${
              isScrolled ? 'border-[#2D5A27]' : 'border-white/50'
            }`}>
              <img 
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navLinks.map((link) => (
              <a
                ...
                className={`nav-item px-3 py-1.5 text-sm font-medium transition-all duration-300 rounded-full hover:bg-[#2D5A27]/10 ${
                  isScrolled 
                    ? 'text-[#2D5A27] hover:text-[#1B3A16]' 
          ...
          <Button
            ...
            className={`hidden sm:flex nav-item rounded-full px-5 py-2 text-sm font-semibold transition-all duration-300 ${
              isScrolled
                ? 'bg-[#2D5A27] text-white hover:bg-[#1B3A16]'
        ...
        <div className="flex flex-col items-center justify-center h-full gap-6">
          {navLinks.map((link, index) => (
            <a
              ...
              href={link.href}
              onClick={(e) => { e.preventDefault(); scrollToSection(link.href); }}
              className="text-2xl font-outfit font-medium text-[#2D5A27] hover:text-[#CE1126] transition-colors"
          ...
          <Button
            onClick={() => scrollToSection('#agenda')}
            className="mt-4 bg-[#CE1126] text-white hover:bg-[#A00D1E] rounded-full px-8 py-3 text-lg"
          >
            Agendar Cita
          </Button>
        </div>
